    ...
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="fw-bold mb-0 text-dark tracking-tight">
                <i class="bi bi-pencil-square text-primary me-2"></i> Modify Transaction
            </h2>
        </div>
    </x-slot>
                ...
                <div class="col-12 col-lg-8">
                    <div class="glass-card card p-4 p-md-5">
                        <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
                            <h4 class="fw-bold mb-0 text-dark">Update Details</h4>
                        ...
                        <form action="{{ route('transactions.update', $transaction) }}" method="POST">
                            ...
                            <div class="row g-4 mb-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select form-select-lg bg-light border-0 shadow-none @error('type') is-invalid @enderror" id="type" name="type" required>
                                        <option value="expense" {{ old('type', $transaction->type) == 'expense' ? 'selected' : '' }}>Expense (Spending)</option>
                                        <option value="income" {{ old('type', $transaction->type) == 'income' ? 'selected' : '' }}>Income (Earnings)</option>
                                    </select>
                                    @error('type')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" class="form-control form-control-lg bg-light border-0 shadow-none @error('date') is-invalid @enderror" id="date" name="date" value="{{ old('date', \Carbon\Carbon::parse($transaction->date)->format('Y-m-d')) }}" required>
                                    @error('date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control form-control-lg bg-light border-0 shadow-none @error('category') is-invalid @enderror" id="category" name="category" value="{{ old('category', $transaction->category) }}" placeholder="e.g., Groceries, Rent, Salary" required>
                                    @error('category')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group input-group-lg">
                                        ...
                                        <input type="number" step="0.01" min="0.01" class="form-control bg-light border-0 shadow-none @error('amount') is-invalid @enderror" id="amount" name="amount" value="{{ old('amount', floatval($transaction->amount)) }}" placeholder="0.00" required>
                                        @error('amount')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                            ...
                            <div class="d-flex justify-content-end mt-4 pt-3 border-top">
                                <button type="submit" class="btn btn-primary btn-lg rounded-pill px-5 shadow-sm fw-semibold">
                                    Finalize Update <i class="bi bi-save ms-2"></i>
                                </button>
                            </div>
                        </form>
                    </div>
